...
def measureLossGradient(data_loader):
    model.eval()
    gradSize=list(test_loader)[0][0].numel()
    meanGrad=torch.zeros(gradSize) #Torch with (B x n_input elements), since the gradient vector has that number of elements
    meanGrad2=0
    meanLoss=0
    meanLoss2=0
    for data, target in data_loader:
        ...
        loss.backward()
        gradient=data.grad.data.view(gradSize)
        meanGrad+=gradient
        meanGrad2+=gradient.dot(gradient)
    meanLoss/=len(data_loader)
    meanLoss2/=len(data_loader)
    meanGrad/=len(data_loader)
    meanGrad2/=len(data_loader)
    print("<L>: ", meanLoss)
    print("<L^2>: ", meanLoss2)
    print("var(L) = <L^2>-<L>^2", meanLoss2-meanLoss*meanLoss)
    print("<grad>^2",meanGrad.dot(meanGrad))
    print("<grad^2>",meanGrad2)
    print("var(grad) = <grad^2>-<grad>^2",meanGrad2-meanGrad.dot(meanGrad))
