...
def train(period,n_step = 1000,lr=0.1):
    ...
    for batch_idx, (data, target) in enumerate(circ_train_loader):
        ...
        optimizer.zero_grad()
        output = model(data)
        loss = bruna_loss10(output, target)+bruna_decay*(L1_regularization(model)+L2_regularization(model))
        loss.backward()
        ...
        if batch_idx % args.print_interval == 0:
            ...
            sys.stdout.flush() #Si puo togliere quando avro capito che cosa cazzo non va col cluster
        if batch_idx==n_step:
            break
